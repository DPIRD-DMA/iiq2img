"""
Fast IIQ demosaic research — progress dump.

Findings
--------
1. rawpy.black_level_per_channel incorrectly reports [0,0,0,0] for Phase One IIQ.
   Actual black level is 1024 ADU — confirmed by tracing individual raw pixels against
   LibRaw's linear 16-bit output: raw=1028 → LibRaw=4 (diff=1024 exactly).

2. After subtracting black=1024 the Bayer data is LINEAR with LibRaw's output.
   No additional tone/linearisation curve is needed for this sensor.

3. cv2.COLOR_BAYER_RG2BGR on uint16 RGGB input produces RGB-ordered output (R=ch0),
   NOT BGR. Do NOT apply [:,:,::-1] reversal.

4. Auto-brightness: LibRaw clips ~0.1% of subsampled luma pixels (not 1%).

5. Numba parallel JIT on the LUT application step drops from 380ms → 172ms.

Pipeline
--------
  raw_image_visible  →  numba WB+black LUT on Bayer  →  cv2 EA demosaic (uint16)
  →  subsampled luma auto-bright threshold  →  build per-channel combined LUT
  (black+WB+auto-bright+sRGB-gamma, uint16→uint8)  →  numba LUT3 application

Timing (Phase One iXM-GS120, 120MP, 12768×9564):
  LibRaw PPG (original):  ~2900 ms
  Fast pipeline:           ~550 ms   →  5.3× speedup
  Quality: mean abs diff ~10/255 vs LibRaw PPG
"""
...
def _lut3(img: np.ndarray, lr: np.ndarray, lg: np.ndarray, lb: np.ndarray) -> np.ndarray:
...
@numba.njit(parallel=True)
def _wb_lut_bayer(bayer: np.ndarray,
...
def _warmup_numba() -> None:
    ...
    lut = np.arange(256, dtype=np.uint8).repeat(256)[:65536]
    _lut3(dummy16, lut, lut, lut)
    _wb_lut_bayer(dummy_bayer, lut[:65536].view(np.uint16),
                  lut[:65536].view(np.uint16), lut[:65536].view(np.uint16))
...
def _build_gamma_lut(wb_scale: float, threshold: float) -> np.ndarray:
    """Build a uint16→uint8 LUT combining WB scale + auto-bright + sRGB gamma."""
    x = np.arange(65536, dtype=np.float32) * wb_scale / (threshold * 65535.0)
...
def demosaic_fast(iiq_path: str | Path, clip_frac: float = 0.001) -> np.ndarray:
    """
    ...
    clip_frac : auto-brightness highlight clip fraction (default 0.1%)
    ...
    # Auto-brightness threshold on subsampled luma
    ...
    # Combined LUT: auto-bright + sRGB gamma (uint16→uint8).
    # WB is already applied in the Bayer step so scale=1.0 for all channels.
    gamma_lut = _build_gamma_lut(1.0, threshold)
    lut_r = lut_g = lut_b = gamma_lut

    # Apply LUTs (numba parallel)
    return _lut3(rgb16, lut_r, lut_g, lut_b)
...
def main() -> None:
    ...
    print(f"  {'Fast (numba+cv2 EA)':<30} {fast_ms:>7.0f}ms  {libraw_ms/fast_ms:>9.2f}×")
    ...
    print(f"  Per-channel:   R={abs_diff[:,:,0].mean():.2f}  "
          f"G={abs_diff[:,:,1].mean():.2f}  B={abs_diff[:,:,2].mean():.2f}")
    print(f"  Output means — fast: R={rgb_fast[:,:,0].mean():.1f}  "
          f"G={rgb_fast[:,:,1].mean():.1f}  B={rgb_fast[:,:,2].mean():.1f}")
    print(f"  Output means — ref:  R={rgb_ref[:,:,0].mean():.1f}  "
    ...
    for name, (r0, c0, r1, c1) in {
        ...
        cv2.putText(sbs, "LibRaw PPG",    (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 0), 2)
        cv2.putText(sbs, "Fast (EA+LUT)", (sbs.shape[1]//2 + 10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 0), 2)
        cv2.imwrite(str(output_dir / f"compare_{name}.jpg"),
                    cv2.cvtColor(sbs, cv2.COLOR_RGB2BGR),
                    [cv2.IMWRITE_JPEG_QUALITY, 92])

    cv2.imwrite(str(output_dir / "libraw_ppg.jpg"),
                cv2.cvtColor(rgb_ref,  cv2.COLOR_RGB2BGR),
                [cv2.IMWRITE_JPEG_QUALITY, 92])
    cv2.imwrite(str(output_dir / "fast_demosaic.jpg"),
